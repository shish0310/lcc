...
int main() {
  ...
  double_g_val->setInitializer(
      ConstantFP::get(irBuilder->getDoubleTy(), "1.22"));

  StructType *structType = StructType::create(*llvmContext, "struct_type");
  structType->setBody({irBuilder->getInt32Ty(), irBuilder->getDoubleTy(),
  ...
  struct_g_val->setInitializer(ConstantStruct::get(
      structType,
