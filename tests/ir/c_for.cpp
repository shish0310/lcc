...
int main() {
  ...
  Function *sumFunc =
      Function::Create(fType, GlobalVariable::ExternalLinkage, "sum", *module);
  std::vector<std::string> param{"a", "b"};
  int idx = 0;
  for (Function::arg_iterator it = sumFunc->arg_begin();
       it != sumFunc->arg_end(); ++it, ++idx) {
    it->setName(param[idx]);
  }
  BasicBlock *entryBB = BasicBlock::Create(*llvmContext, "entry", sumFunc);
  BasicBlock *loop_initBB =
      BasicBlock::Create(*llvmContext, "loop_init", sumFunc);
  BasicBlock *loop_condition =
      BasicBlock::Create(*llvmContext, "loop_condition", sumFunc);
  BasicBlock *loop = BasicBlock::Create(*llvmContext, "loop", sumFunc);
  BasicBlock *loop_increment =
      BasicBlock::Create(*llvmContext, "loop_increment", sumFunc);
  ...
  Value *diff =
      irBuilder->CreateSub(sumFunc->getArg(0), sumFunc->getArg(1), "diff");
  ...
  Value *cnt_val =
      irBuilder->CreateLoad(irBuilder->getInt32Ty(), cnt, "cnt_val");
  Value *condition_val =
      irBuilder->CreateICmpSLT(i_val, cnt_val, "condition_val");
  ...
  Value *ret_val_t =
      irBuilder->CreateAdd(ret_val, irBuilder->getInt32(1), "ret_val_t");
  ...
  Value *i_val_t =
      irBuilder->CreateAdd(i_val, irBuilder->getInt32(1), "i_val_t");
